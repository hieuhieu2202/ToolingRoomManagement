...
{
    public class StationRobotController : Controller
    {
        // GET: Admin/StationRobot
        ToolingRoomDbContext context = new ToolingRoomDbContext();
        public ActionResult Index()
        {
            return View();
        }
        public string getAllStationRobot()
        {
            var list = new Station_Robot_Dao().listAllStation_Robot();
        ...
        public string Insert(int id, string station_name)
        {
            ...
            {
                int id_station = new Station_Robot_Dao().Insert(station_name);
                if (id_station > 0)
                {
                    return "OK";
                }
